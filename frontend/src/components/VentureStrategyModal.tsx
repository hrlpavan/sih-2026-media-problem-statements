...
export const VentureStrategyModal: React.FC<VentureStrategyModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div 
      className="fixed inset-0 z-50 overflow-y-auto bg-black/70 backdrop-blur-md flex items-center justify-center p-3 sm:p-6 md:p-8 animate-in fade-in duration-150"
      onClick={onClose}
    >
      {/* Floating Redundant Close Button */}
      <button
        onClick={onClose}
        className="fixed top-4 right-4 z-50 p-2.5 rounded-full bg-white/90 hover:bg-white text-zinc-800 shadow-xl border border-black/10 cursor-pointer transition-all hover:scale-105"
        title="Close (Esc)"
      >
        <X className="w-5 h-5" />
      </button>
      ...
      {/* Main Modal Card */}
      <div 
        className="relative bg-white rounded-[24px] sm:rounded-[28px] border border-black/10 shadow-2xl max-w-4xl w-full max-h-[85vh] flex flex-col overflow-hidden my-auto"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Sticky Modal Header */}
        <div className="p-5 sm:p-6 border-b border-zinc-100 bg-white/95 backdrop-blur-md flex items-center justify-between sticky top-0 z-10">
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-wider text-hrl-crimson bg-hrl-crimson-tint px-2.5 py-0.5 rounded-full">
